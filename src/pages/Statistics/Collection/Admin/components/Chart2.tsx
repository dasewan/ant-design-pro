...
const Chart2: React.FC<FormProps> = (props) => {
  ...
  let periodFlags: string[] = [];
  if (!Array.isArray(props.dimensions)) {
    return <></>;
  }
  props.dimensions.map((dimension) => {
    periodFlags.push(dimension[0]);
    props.rawData.map((item) => {
      lineData.push({
        time: moment(item.a_date).format('MM/DD'),
        value: item[dimension],
        name: FieldLabels[dimension],
      });
      return item;
    });
    ...
    pieData.push({
      name: FieldLabels[dimension],
      value: Number(_.sumBy(props.rawData, dimension).toFixed(1)),
    });

    return dimension;
  });
  periodFlags = _.uniq(periodFlags);
  periodFlags.map((periodFlag) => {
    _.chain(props.rawData)
      .groupBy(periodFlag)
      .map((item) => {
        columnData2.push({
          name: PERIOD_FLAG[periodFlag],
          value: Number(
            (
