import { PERIODS, RISK_TAGS_GROUP_ARRAY } from '@/pages/enums';

import Chart3 from '@/pages/Statistics/Risk/RiskTag/components/Chart3';
...
import { getAdminV1WKRiskTags as index } from '@/services/ant-design-pro/WKRiskTag';
  ...
  return (
    ...
    >
        ...
        <div>
          ...
          >
            <ProForm.Group>
              ...
              />
              <ProFormSelect
                options={RISK_TAGS_GROUP_ARRAY}
                name="d_risk_tag_group"
                label="字段"
                rules={[{ required: true }]}
              />
              <ProFormSelect
      ...
      <Spin spinning={loading}>
        <div key="1">
          <Chart3 rawData={records} period={period} dpd={dpd} riskTags={riskTags}></Chart3>
          {_.chain(records)
            .groupBy('c_risk_tag_id')
            .map((item, index) => {
              return (
                <>
                  <div>
                    {
                      riskTags!.find((item2) => item2.value === item[0].c_risk_tag_id!)!
                        .label as string
                    }
                  </div>
