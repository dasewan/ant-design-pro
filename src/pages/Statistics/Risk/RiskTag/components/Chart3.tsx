...
const Chart3: React.FC<FormProps> = (props) => {
  let lineData = Object.values(
    props.rawData!.reduce((acc: { [key: string]: any }, curr: { [key: string]: any }) => {
      let tagName = props.riskTags!.find((item2) => item2.value === curr.c_risk_tag_id)!
        .label as string;
      const key = `${curr.c_risk_tag_id}-${curr.a_risk_date}`;
      if (!acc[key]) {
        acc[key] = {
          name: tagName,
          count: curr.f_count,
  ...
  if (props.dpd === 'dpd1') {
    if (props.period === '1') {
      _.chain(props.rawData)
        ...
        .map((item) => {
          pieData.push({
            name: props.riskTags!.find((item2) => item2.value === item[0]!.c_risk_tag_id)!
              .label as string,
      ...
      props.rawData.map((item) => {
        lineData.push({
          ...
          name: item.c_risk_tag_id!.toString(),
        });
        return item;
      });
