  ...
  return (
    ...
    >
      ...
      <ProFormSelect
        label="接收小组"
        tooltip={<></>}
        name="after_collection_group_id"
        rules={[{ required: true, message: `请选择接收小组` }]}
        // @ts-ignore
        options={props.collectionGroups.filter(
          (item) =>
            //todo 阶段有交集
            item.c_collection_agency_id === props.record?.c_collection_agency_id &&
            item.f_status === 1 &&
