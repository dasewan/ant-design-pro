...
const TableList: React.FC = () => {
  ...
  const _index = async (
    ...
    // 如果需要转化参数可以在这里进行修改
    // @ts-ignore
    const res = await index({ page: params.current, type: 2, ...params });
    return {
      data: res.data,
      // success 请返回 true，
  ...
  const columns: ProColumns<TableListItem>[] = [
    ...
    },
    {
      ...
      // render: (_, record) => {
      //   return Math.ceil(moment(record.a_a_a_a_a_a_user!.created_at).diff(moment(record!.created_at))/(1000*60*69*24))
      // },
      // hideInSearch: true
    },
    {
    ...
    },
    {
      ...
      fieldProps: { placeholder: '支持区间' },
      search: {
        transform: (value: any) => ({ 'a_user-a_f_loan_count': value }),
      },
    },
    {
      title: '逾期次数',
