...
const TableList: React.FC = () => {
  ...
  const _index = async (
    ...
    // 如果需要转化参数可以在这里进行修改
    // @ts-ignore
    const res = await index({ page: params.current, f_status: 60, ...params });
    // @ts-ignore
  ...
  const columns: ProColumns<TableListItem>[] = [
    ...
    {
      title: intl.formatMessage({ id: 'pages.Loan.n_error_message2', defaultMessage: '' }),
      dataIndex: 'n_error_message',
      ellipsis: true,
      width: 120,
    },
    {
      title: intl.formatMessage({ id: 'pages.Loan.updated_at2', defaultMessage: '' }),
      ...
      render: (_, record) => {
        const transfer = (
          <Popconfirm
            title={
              <>
                This transfer has hit loan risk,Are you sure to transfer
