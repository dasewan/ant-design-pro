...
class ServiceGenerator {
    ...
    getFuncationName(data) {
        ...
        let r = this.config.hook && this.config.hook.customFunctionName
    ...
    getFinalFileName(s) {
      ...
      let str = s.replace(/[-_ ](\w)/g, (_all, letter) => letter.toUpperCase());
      // 获取前两个字符并转换为大写
      const firstTwoChars = str.slice(0, 2).toUpperCase();

      // 获取剩余部分
      const remainingChars = str.slice(2);
